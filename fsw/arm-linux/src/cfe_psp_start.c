...
void OS_Application_Startup(void)
{
   ...
   */
   ...
   /*
   ** Set up virtual FS mapping for the "/a" directory
   ** This folder is for transfers from the SNOOPI DBE. It is mapped
   ** to a single character cFS directory due to the long filenames from
   ** the DBE, requiring as short of a path as possible.
   */
   Status = OS_FileSysAddFixedMap(&fs_id, "/mnt/nvmem/from_dbe", "/a");
   if (Status != OS_SUCCESS)
   {
       /* Print for informational purposes --
        * startup can continue, but loads may fail later, depending on config. */
   ...
   /*
   ** Set up virtual FS mapping for the "/verified_overlay" directory
   ** This folder is for in-flight cFS updates that have been tested and verified.
   ** The contents will overwrite the contents of the /root/cf folder on system startup
   */
   Status = OS_FileSysAddFixedMap(&fs_id, "/mnt/nvmem/verified_overlay", "/verified_overlay");
   if (Status != OS_SUCCESS)
   {
       /* Print for informational purposes --
        * startup can continue, but loads may fail later, depending on config. */
   ...
   /*
   ** Set up virtual FS mapping for the "/tmp_overlay" directory
   ** This folder is for in-flight cFS updates that have yet to be tested and verified.
   */
   Status = OS_FileSysAddFixedMap(&fs_id, "/mnt/nvmem/tmp_overlay", "/tmp_overlay");
   if (Status != OS_SUCCESS)
   {
       /* Print for informational purposes --
        * startup can continue, but loads may fail later, depending on config. */
   ...
   /*
   ** Set up virtual FS mapping for the "/update_scratch" directory
   ** This folder is for in-flight cFS updates that have been copied to /root/cf, waiting to be verified.
   ** This exists as a convenience for operators to save upload bandwidth if an update is proven to work,
   ** before being copied to verified_overlay.
   */
   Status = OS_FileSysAddFixedMap(&fs_id, "/mnt/nvmem/update_scratch", "/update_scratch");
   if (Status != OS_SUCCESS)
   {
       /* Print for informational purposes --
        * startup can continue, but loads may fail later, depending on config. */
   ...
   */
   Status = OS_FileSysAddFixedMap(&fs_id, "/mnt/nvmem/data", "/data");
   if (Status != OS_SUCCESS)
   {
       /* Print for informational purposes --
        * startup can continue, but loads may fail later, depending on config. */
