...
#define FPGA_INTERRUPT_ID_1PPS_EVENT    (11u)
...
static  void CFE_PSP_1HzInterruptHandler(void)
{
  CFE_TIME_Tone1HzISR();
}
...
void CFE_PSP_Setup1HzInterrupt(void)
{
    ...
    Status = rtems_interrupt_handler_install((rtems_vector_number)FPGA_INTERRUPT_ID_1PPS_EVENT, "!Hz", RTEMS_INTERRUPT_UNIQUE,  (rtems_interrupt_handler) CFE_PSP_1HzInterruptHandler, 0);

    if (Status != RTEMS_SUCCESSFUL)
    {
        printf("\n Failed to attach handler to 1Hz interrrupt");
        return Status;
    }
}
